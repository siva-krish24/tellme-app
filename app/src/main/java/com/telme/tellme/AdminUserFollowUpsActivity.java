...
import com.telme.tellme.reclyeradpaters.admin.AdminUserFollowupsRecyclerAdapter;
...
public class AdminUserFollowUpsActivity extends AppCompatActivity {
    private RecyclerView mRecyclerview;
    private List<Object> viewItems = new ArrayList<>();
    private RecyclerView.Adapter mAdapter;
    private RecyclerView.LayoutManager layoutManager;
    private static final String TAG = "AUFollowUpsActivity";
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        try {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_admin_user_follow_ups);

        mRecyclerview =  findViewById(R.id.admin_user_followup_recycler_view);
        mRecyclerview.setHasFixedSize(true);

        layoutManager = new LinearLayoutManager(this);
        mRecyclerview.setLayoutManager(layoutManager);

        mAdapter = new AdminUserFollowupsRecyclerAdapter(this,viewItems);
        mRecyclerview.setAdapter(mAdapter);

        StrictMode.ThreadPolicy policy = new StrictMode.ThreadPolicy.Builder().permitAll().build();
        StrictMode.setThreadPolicy(policy);
        UserCustomeRestUtil.fetchData(getIntent().getExtras( ),"getintrested",viewItems);
    ...
    private void fetchData() throws IOException, JSONException {
        ...
        String response = getData("http://192.168.29.120:8080/getintrested",userstr);
        Log.d(TAG, response);
